  ...
  return (
    <fieldset className='flex flex-col items-start gap-1 w-full'>
        ...
        <div className='relative w-full'>
            <input type={show ? "text" : "password"} name={name} id={id} value={value} className={`z-0 bg-brand-gray-one/30 pr-4 border border-brand-gray-one/30 rounded-sm px-2 py-2 w-full`} />
            <button type='button' onClick={()=>setShow(prev => !prev)} className='cursor-pointer z-10 absolute top-1/2 -translate-y-1/2 right-2 w-fit'>
                ...
                <div className={`bg-brand-purple-one -rotate-45 font-medium h-6 w-0.5 absolute top-1/2 -translate-y-1/2 translate-x-1/2 right-1/2 z-20 ${!show ? "opacity-100" : "opacity-0"}`}></div>
            </button>
        </div>
    </fieldset>
  )
